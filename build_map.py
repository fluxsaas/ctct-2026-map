...
data = {"roads": roads_data, "markers": markers_data}
...
#legend {
  ...
  border-radius:12px; padding:12px 15px; min-width:242px;
  box-shadow:0 2px 14px rgba(0,0,0,0.15);
  max-height:90vh; overflow-y:auto;
}
#legend h3 { font-size:13px; font-weight:700; margin-bottom:2px; }
#legend .sub { font-size:10px; color:#888; margin-bottom:9px; }
...
<div id="legend">
  ...
  <hr/>
  <div class="li"><div class="ld" style="border-color:#1a73e8"></div><strong>My Route</strong> (fastest, 12:15)</div>
  <hr/>
...
function renderRoads(data) {
  data.roads.forEach(r => {
    ...
    L.polyline(r.coords, opts).addTo(map).bindPopup(
